...
class FollowService {
  ...
  private async executeFollowOperationFast(currentUserId: string, targetUserId: string): Promise<boolean> {
    try {
      ...
      // Update user profile counts immediately (fire and forget for speed)
      this.updateProfileCountsAfterFollow(currentUserId, targetUserId);
  ...
  /**
   * Update user profile counts after follow (non-blocking)
   */
  private updateProfileCountsAfterFollow(currentUserId: string, targetUserId: string): void {
    (async () => {
      try {
        // Import what we need
        const { increment, updateDoc } = await import('firebase/firestore');
        
        // Update both user profiles in parallel
        await Promise.all([
          updateDoc(doc(db, 'users', currentUserId), { following: increment(1) }),
          updateDoc(doc(db, 'users', targetUserId), { followers: increment(1) })
        ]);
        
        console.log('✅ Profile counts updated after follow');
      } catch (error) {
        console.warn('⚠️ Failed to update profile counts after follow:', error);
      }
    })();
  }
  ...
  private async executeUnfollowOperationFast(currentUserId: string, targetUserId: string): Promise<boolean> {
    try {
      ...
      // Update user profile counts immediately (fire and forget for speed)
      this.updateProfileCountsAfterUnfollow(currentUserId, targetUserId);
  ...
  /**
   * Update user profile counts after unfollow (non-blocking)
   */
  private updateProfileCountsAfterUnfollow(currentUserId: string, targetUserId: string): void {
    (async () => {
      try {
        // Import what we need
        const { increment, updateDoc } = await import('firebase/firestore');
        ...
        console.log('✅ Profile counts updated after unfollow');
      } catch (error) {
        console.warn('⚠️ Failed to update profile counts after unfollow:', error);
      }
    })();
  }
